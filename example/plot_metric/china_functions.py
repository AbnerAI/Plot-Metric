...
class BinaryClassification:
    ...
    def plot_roc_curve(self, threshold=None, plot_threshold=True, linewidth=2, y_text_margin=0.05, x_text_margin=0.2,
        ...
        plt.plot(fpr, tpr, color=c_roc_curve,
                 lw=linewidth, label='ROC 曲线 (面积 = %0.2f)' % roc_auc, linestyle=ls_roc_curve)

        # Plot reference line
        plt.plot([0, 1], [0, 1], color=c_random_guess, lw=linewidth, linestyle=ls_random_guess, label="随机猜测")
        # Plot threshold
        if plot_threshold:
    ...
    def plot_precision_recall_curve(self, threshold=None, plot_threshold=True, beta=1, linewidth=2,
        ...
        lines.append(l)
        labels.append('PR 曲线 (面积 = {})'.format(round(pr_auc, 2)))

        # Plot mean precision
        l, = plt.plot([0, 1], [mean(prec), mean(prec)], color=c_mean_prec,
                      lw=linewidth, linestyle=ls_mean_prec)
        lines.append(l)
        labels.append('平均精度 = {}'.format(round(mean(prec), 2)))
    ...
    def plot_class_distribution(self, threshold=None, display_prediction=True, alpha=.5, jitter=.3, pal_colors=None,
        ...
        def __compute_thresh(row, _threshold):
            if (row['预测'] >= _threshold) & (row['类别'] == 1):
                return "TP"
            elif (row['预测'] >= _threshold) & (row['类别'] == 0):
                return 'FP'
            elif (row['预测'] < _threshold) & (row['类别'] == 1):
                return 'FN'
            elif (row['预测'] < _threshold) & (row['类别'] == 0):
                return 'TN'

        pred_df = pd.DataFrame({'类别': self.y_true, '预测': self.y_pred})

        pred_df['类型'] = pred_df['预测']
        pred_df['类型'] = pred_df.apply(lambda x: __compute_thresh(x, t), axis=1)
        
        # Plot violin pred distribution
        if display_violin:
            sns.violinplot(x='类别', y='预测', data=pred_df, inner=None, color=c_violin, cut=0)
        
        # Plot prediction distribution
        if display_prediction:
            sns.stripplot(x='类别', y='预测', hue='类型', data=pred_df,
        ...
        pred_df['Predicted Class'] = pred_df['预测'].apply(lambda x: self.labels[1] if x >= t else self.labels[0])
        pred_df.columns = ['真实类型', '预测概率', '预测类型', '预测类别']
        return pred_df
    ...
    def plot_score_distribution(self, threshold=None, plot_hist_TN=True, kde_ksw_TN={'shade': True},
        ...
        ax = sns.distplot(TN_pred, hist=plot_hist_TN, kde_kws=kde_ksw_TN, label=label_TN, color=c_TN_curve)
